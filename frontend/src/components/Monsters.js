...
// !!!!!!!!! NEED TO CHANGE THIS BACK TO 2500
  ...
  axios.get(`https://api.open5e.com/monsters/?limit=2450&ordering=${sortBy}`).then((response) => {
    const sortedMonsters = response.data.results.sort((a, b) => {
      if (typeof a[sortBy] === 'number') {
        return a[sortBy] - b[sortBy];
      } else {
...
const [showMonster, setShowMonster] = useState(Currentmonsters.map(monster => false))
...
const removeFromFavorites = (monster) => {
  const updatedFavorites = favorites.filter(favorite => favorite.slug !== monster.slug);
  setFavorites(updatedFavorites);
  localStorage.setItem('favorites', JSON.stringify(updatedFavorites));
  };
...
<select className="drop-down" onChange={(e) => getMonsters(e.target.value)}>
...
<Search className="search-bar"/>
    ...
    {Currentmonsters.map((monster, index) => {
      return (
        ...
        <td>{monster.size}</td>
        <td><FontAwesomeIcon  id="add-to-favorites" icon={faHeart} onClick={() => addToFavorites(index)}/></td>
        <td><a onClick={()=> {
    ...
    <Pagination postsPerPage={postsPerPage} totalPosts={monsters.length} paginate={paginate}/>

    </div>

    <div className={hidden2}>
    <Favorites  favorites={favorites} setFavorites={setFavorites} onRemove={removeFromFavorites}/>
    </div>
